/**
 * Estrutura para o projeto Folha de pontoLeticia mobile
...
function createTable() {
    db.transaction(function(transaction) {
        var sql = "CREATE TABLE IF NOT EXISTS pontoLeticia (id INTEGER PRIMARY KEY AUTOINCREMENT, data DATE NOT NULL, entrada TIME NOT NULL DEFAULT '00:00:00', almoco  TIME NOT NULL DEFAULT '00:00:00', retorno TIME NOT NULL DEFAULT '00:00:00', saida  TIME NOT NULL DEFAULT '00:00:00');";
...
function selectPeriod() {
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM pontoLeticia WHERE data = ?", [currentDate], function(transaction, results) {
...
function updateReg(periodo, currentTime, currentDate) {
    db.transaction(
            function(transaction) {
                transaction.executeSql("UPDATE pontoLeticia SET " + periodo + "=? WHERE data = ?;", [currentTime, currentDate], console.log('updated'), onError);
            }
    );
}
...
function deletarReg(id) {
    db.transaction(
            function(transaction) {
                transaction.executeSql("DELETE FROM pontoLeticia WHERE id = ?;", [id], function() {
...
function registrar() {
    ...
    db.transaction(function(transaction) {
        transaction.executeSql("SELECT * FROM pontoLeticia WHERE data = ?", [currentDate], function(transaction, results) {
            ...
            if (len === 0) {
                transaction.executeSql("INSERT INTO pontoLeticia ('data') VALUES (?)", [currentDate], console.log(currentDate), onError);
                updateReg(periodo, currentTime, currentDate);
                $.mobile.changePage("folha-de-pontoLeticia.html", {transition: "slideup"});
            } else {
                transaction.executeSql("SELECT * FROM pontoLeticia WHERE data = ? AND " + periodo + "<>'00:00:00'", [currentDate], function(transaction, results) {
...
function loadFolha() {
    db.transaction(function(tx) {
        tx.executeSql('SELECT * FROM pontoLeticia ORDER BY data desc', [], function(tx, results) {
            var len = results.rows.length;
...
function loadDiario() {
    db.transaction(function(tx) {
        tx.executeSql('SELECT * FROM pontoLeticia WHERE data=?', [currentDate], function(tx, results) {
            var len = results.rows.length;
...
$(document).ready(function() {
    ...
    $(document).on('pageshow', '#updatemanual', function() {
        ...
        $("#inserirDadosTabelapontoLeticia").on("click", function() {
            batchInsert();
            $.mobile.changePage("folha-de-pontoLeticia.html", {transition: "slideup"});
        }).trigger('create');
    ...
    $(document).on('pageshow', '#folha-de-pontoLeticia', function() {
        loadFolha();
    });
